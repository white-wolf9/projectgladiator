...
public class UserDaoImpl extends GenericDaoImpl implements UserDao {
	...
	public String performLogin(UserLoginDTO userLoginDTO) {
		User user_temp = null;
		try {
			user_temp = (User) entityManager
					.createQuery("select u from User as u where u.credential.username = :username")
					.setParameter("username", userLoginDTO.getUsername()).getSingleResult();
	...
	public String performRegister(UserRegisterDTO userRegisterDTO) {
		...
		/*
		 * All the details of the bank account entered by the user are stored in object
		 * of type BankDetails to be assigned to the User Entity
		 */
		...
		/*
		 * All the details of the address entered by the user is stored in this object
		 * of type Address and is to be assigned to User Entity
		 */
		...
		/*
		 * All the details of the documents entered by the user is stored in this object
		 * of type Document and is assigned to the User Entity
		 */
		User user = new User();
	...
	/*
	 * Shift the viewAllUser() to the administrator DAO because this is a
	...
	public CardDetailsResponseDTO fetchCardUser(CardDetailsRequestDTO cardDetailsRequestDTO) {
		...
		try {
			user_temp = (User) entityManager
					.createQuery("select u from User as u where u.credential.username = :username")
					.setParameter("username", cardDetailsRequestDTO.getUsername()).getSingleResult();
		} catch (NoResultException noResultException) {
	...
	public User fetchByUsername(String username) {
		return (User) entityManager.createQuery("select u from User as u where u.credential.username = :username")
				.setParameter("username", username).getSingleResult();
	}

	@Override
	@Transactional
	public ProductOrderResponseDTO placeOrderFresh(double installment_value, double totalAmtToPay, User user,
			ProductOrderRequestDTO productOrderRequestDTO, Product product) {
	...
	public List<ViewAllOrderResopnseDTO> fetchAllOrder(String username) {
		...
		List<Order> order_temp_list = (List<Order>) entityManager
				.createQuery("select o from Order as o where o.user.credential.username = :username")
	...
	public String payInstallment(InstallmentPaymentRequestDTO installmentPaymentRequestDTO) {
		...
		if (payment.getPaid_installments() < payment.getTotal_installments()
				&& payment.getPayment_status().equals(PaymentStatus.OPEN)) {
			User user = (User) entityManager
					.createQuery("select u from User as u where u.credential.username= :username")
					.setParameter("username", installmentPaymentRequestDTO.getUsername()).getSingleResult();
			// Card card = (Card) entityManager.createQuery("select c from Card as c where
		...
		/*
		 * if(payment.getPaid_installments()>payment.getTotal_installments()) { User
		 * user = (User) entityManager.
		 * createQuery("select u from User as u where u.credential.username= :username")
		 * .setParameter("username",
		 * installmentPaymentRequestDTO.getUsername()).getSingleResult(); //Card card =
		 * (Card) entityManager.
		 * createQuery("select c from Card as c where c.user.credential.username = :username"
		 * ).setParameter("username", installmentPaymentRequestDTO.getUsername()); Card
		 * card =user.getCard();
		 * card.setCard_balance(card.getCard_balance()-payment.getInstallment_value());
		 * entityManager.merge(card);
		 * payment.setPaid_installments(payment.getPaid_installments()+1);
		 * payment.setPayment_status(PaymentStatus.CLOSE); entityManager.merge(payment);
		 * return "All monthly installments were paid"; }
		 */
		payment.setPayment_status(PaymentStatus.CLOSE);
		entityManager.merge(payment);
		return "All installments paid";
